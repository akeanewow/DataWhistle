...
FROM (SELECT
  SUM(CASE WHEN REGEXP_CONTAINS({columnname}, r"{regex_rule}")
...
def colcheck_regex(datasetname: str,
                   tablename: str,
                   columnname: str,
    ...
    if regex_type == 'mandatory' and row_fraction_match < 1:
        return False, (f'column {columnname} found a non matching regex '
                       f'record with rule {regex_rule}')
    elif regex_type == 'exclude' and row_fraction_match > 0:
        return False, (f'column {columnname} found invalid regex with rule '
                       f'{regex_rule}')
    return True, ''
